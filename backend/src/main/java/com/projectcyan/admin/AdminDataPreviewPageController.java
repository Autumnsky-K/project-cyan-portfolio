...
public class AdminDataPreviewPageController {
	...
	public String dataPreview(Model model) {
		model.addAttribute("dataAreas", List.of(
			new DataArea("상품 데이터", "상품명, 가격, 태그, 판매상태, 재고를 관리합니다.", "/admin/goods", "운영 가능"),
			...
			new DataArea("회원 데이터", "회원 프로필과 권한 데이터 영역입니다.", "", "준비 중"),
			new DataArea("배송 데이터", "배송지, 배송상태, 주문 배송 데이터 영역입니다.", "", "준비 중")
		));
