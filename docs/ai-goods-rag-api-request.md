...
| `targetType` | string | `ARTIST`, `TAG`, `CATEGORY` 중 하나 |
| `targetId` | bigint/null | `artistId`, `categoryId`처럼 ID가 있는 대상이면 사용 |
| `targetValue` | string/null | `PHOTOCARD` 같은 tagName 또는 canonical text |

예시 데이터:

| aliasText | targetType | targetId | targetValue |
|-----------|------------|----------|-------------|
| `에스파` | `ARTIST` | `7` | `aespa` |
| `aespa` | `ARTIST` | `7` | `aespa` |
| `포카` | `TAG` | null | `PHOTOCARD` |
| `포토카드` | `TAG` | null | `PHOTOCARD` |
| `photocard` | `TAG` | null | `PHOTOCARD` |
| `키링` | `CATEGORY` | `3` | `키링` |

권장 제약:

```sql
unique (alias_text, target_type, target_value)
```
...
사용자 요청: 에스파 포카 추천해줘
추출/확장:
- 에스파 -> ARTIST artistId=7, targetValue=aespa
- 포카 -> TAG targetValue=PHOTOCARD
```
